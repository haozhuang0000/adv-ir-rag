...
"""
This script directly from MinerU: https://github.com/opendatalab/MinerU/blob/master/demo/demo.py
"""
...
def do_parse(
    ...
    pdf_file_names: str,  # List of PDF file names to be parsed
    ...
    if backend == "pipeline":
        ...
        model_json = copy.deepcopy(infer_results[idx])
        pdf_file_name = pdf_file_names
        ...
        if f_dump_md:
            ...
            md_writer.write_string(
                f"{pdf_file_name}_{md_name}.md",
                md_content_str,
            )
            # temp_dict = {f'{pdf_file_name}_{md_name}': md_content_str}
            # md_content_str_list.append(temp_dict)
...
def parse_doc(
    ...
    try:
        ...
        pdf_bytes_list = [new_pdf_bytes] ## convert to list as it require list as input
        lang_list = [lang]

        md_content_str = do_parse(
        ...
        md_content_str = '\n'.join([i for i in md_content_str.split('\n') if '.jpg' not in i])
        return md_content_str
    except Exception as e:
        logger.exception(e)

def parse_multi_docs(
        path_list: list[Path],
        output_dir,
        lang="en",
        backend="pipeline",
    ...
    try:
        ...
        #     md_content_str = do_parse(
        #         output_dir=output_dir,
        #         pdf_file_names=file_name_list[idx],
        #         pdf_bytes_list=pdf_bytes_list,
        #         p_lang_list=lang_list,
        #         backend=backend,
        #         parse_method=method,
...
if __name__ == '__main__':
    ...
    parse_doc(pdf_bytes_list[0],
              ...
              lang=lang_list[0],
              backend="pipeline", start_page_id=0, end_page_id=3, md_name='contents')
